...
class MavlinkManager:
    ...
    def _initialize_the_port(self):
        ...
        if not self.port_connected:
            ...
            self.mav_connection2 = mavutil.mavlink_connection("udpin:127.0.0.1:14553")
            # self.mav_connection = mavutil.mavlink_connection('tcp:127.0.0.1:5760')

        self.mav_connection.target_system = 1
        self.mav_connection2.target_system = 2
...
def main():
    ...
    mav_connection2 = mav_mgr.mav_connection2
    ...
    # fc2 = FltCtlCommander(mav_connection2)
    ...
    print(
        "Heartbeat from : "
        + str(mav_connection2.target_system)
        + " / "
        + str(mav_connection2.target_component)
    )
    ...
    location1 = {"lat": 39.01784, "lon": -104.89323, "alt": 2180.0}
    location2 = {"lat": 39.017842738, "lon": -104.89323, "alt": 2180.0}
    location3 = {"lat": 39.017845476, "lon": -104.89323, "alt": 2180.0}
    location4 = {"lat": 39.017848214, "lon": -104.89323, "alt": 2180.0}
    location5 = {"lat": 39.017850952, "lon": -104.89323, "alt": 2180.0}

    location6 = {"lat": 39.01784, "lon": -104.89388, "alt": 2180.0}
    location7 = {"lat": 39.01784, "lon": -104.90, "alt": 2180.0}
    location_bank = [
        location1,
        location2,
        location3,
    ...
    while 1:
        ...
        ***IMPORTANT***
        ...
        if count % 20 == 0:
            fc.send_guided_target_to_flt_ctlr(location_bank[i])
            # fc.send_guided_target_to_flt_ctlr(location_bank[random.randint(0,5)])
            i += 1
            if i >= 5:  # uses first 5 iterations
                i = 0

        # fc.send_guided_target_to_flt_ctlr(location1)
        ...
        msg2 = mav_connection2.recv_match(
            type=["GLOBAL_POSITION_INT", "POSITION_TARGET_GLOBAL_INT"], blocking=True
        )
        print(msg2)
        ...
        msg_type2 = msg2.get_type()
        if msg_type2 == "GLOBAL_POSITION_INT":
            # if msg_type == 'POSITION_TARGET_GLOBAL_INT':
            print(msg2)
